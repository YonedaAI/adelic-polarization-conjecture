...
Suppose $\mathcal G_0:\T\to H_0$ and
$\mathcal G_1:\T\to H_1$ are complex-linear maps satisfying the
original compatibility $\mathcal G_1=J\mathcal G_0$. Replacing $J$
by $U$ while retaining both test maps requires
\begin{equation}
 A^{-1/2}\mathcal G_0f=\mathcal G_0f\qquad(f\in\T).
 \label{eq:test-rigidity}
\end{equation}

\begin{proposition}
\label{prop:test-rigidity}
If $\mathcal G_0(\T)$ is dense in $H_0$, the original and polar
test-map compatibilities both hold exactly when $A=\Id$.
Without density they require $A$ to be the identity on the
closure of $\mathcal G_0(\T)$.
\end{proposition}
\begin{proof}
The difference $A^{-1/2}-\Id$ is bounded. If it vanishes on
$\mathcal G_0(\T)$, it vanishes on its closure. On that closure,
...
changed image. Redefining $\mathcal G_1=U\mathcal G_0$ proves an
...
common Hilbert carrier, put $\mathcal H_S=(H,q_S)$,
$V_S=C_S^{-1/2}:H_0\to\mathcal H_S$ and
...
$\mathcal G_S:\mathcal D\to P_S$,
$\mathcal G_T,\eta:\mathcal D\to P_T$ such that
\begin{align}
 \mathcal G_Tf&=\iota\mathcal G_Sf+\eta f,
...
symmetry. Therefore
\[
 I_T(\mathcal G_Tf,\mathcal G_Tf)-I_S(\mathcal G_Sf,\mathcal G_Sf)
...
$\mathcal G_T=\iota\mathcal G_S+\eta$, without orthogonality.
Then the diagonal increment is
\begin{equation}
 I_T(\mathcal G_Tf,\mathcal G_Tf)-I_S(\mathcal G_Sf,\mathcal G_Sf)
 =2\operatorname{Re}I_T(\iota\mathcal G_Sf,\eta f)
 +I_T(\eta f,\eta f).
...
$\iota\mathcal G_Sf+\eta f$ with itself into four terms.
...
as $\eta f=\mathcal G_{T,b}f-\iota\mathcal G_{S,a}f$.

The exact pairing difference is then
\begin{align}
 &I_{T,b}(\mathcal G_{T,b}f,\mathcal G_{T,b}g)
       -I_{S,a}(\mathcal G_{S,a}f,\mathcal G_{S,a}g)\notag\\
 &\quad=E(f,g)
   +I_{T,b}(\iota\mathcal G_{S,a}f,\eta g)
   +I_{T,b}(\eta f,\iota\mathcal G_{S,a}g)
   +I_{T,b}(\eta f,\eta g),
...
\begin{equation}
 B_W(f,g)=-I(\mathcal G f,\mathcal G g)\qquad(f,g\in\T)
